...
### 文字数カウント
　改行コードを除いた文字数をカウントします。

### 文字列検索（おまけ）
　Ctrl + Fで、現在表示しているメモの本文を検索できます。EnterまたはF3で次の一致、Shift + EnterまたはShift + F3で前の一致へ移動します。Escで検索バーを閉じます。  
　メモ帳内の全検索になるため、ファイル名テキストボックスやボタンの文字列も検索対象となる点はご容赦ください。
